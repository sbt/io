...
object Glob {
  ...
  private sealed trait Pattern extends Glob {
    def root: Path
  ...
  def apply(file: File): Glob = Root(file.toPath)
  def apply(file: File, relative: RelativeGlob): Glob = Pattern(file.toPath, relative)
  def apply(path: Path): Glob = Root(path)
  def apply(path: Path, relative: RelativeGlob): Glob = Pattern(path, relative)
  ...
  private[this] def errorMessage(relative: RelativeGlob, warn: Boolean): String = {
  ...
  implicit class GlobOps(val glob: Glob) extends AnyVal {
    ...
    private[sbt] def base(implicit option: RelativeGlobViewOption): Path = glob match {
      case Pattern(root, _) => root
      case Root(root)       => root
      case r: RelativeGlob =>
        option match {
          case Error  => throw new IllegalArgumentException(errorMessage(r, warn = false))
          case Ignore => Paths.get("").toAbsolutePath
          case Warn =>
            System.err.println(errorMessage(r, warn = true))
    ...
    def /(that: RelativeGlob): Glob = glob match {
      ...
      case Root(path) =>
        that match {
          case PathComponent(component) => Root(path.resolve(component))
          case t                        => Pattern(path, t)
        }
  ...
  final class PathOps(val path: Path) extends AnyVal {
    def glob: Glob = Root(path)
    def /(component: String): Path = path.resolve(component)
    def /(relative: RelativeGlob): Glob = Pattern(path, relative)

  }
  final class FileOps(val file: File) extends AnyVal {
    def glob: Glob = Root(file.toPath)
    def /(relative: RelativeGlob): Glob = Pattern(file.toPath, relative)
  }
...
object RelativeGlob {
  ...
  def apply(matchers: String*): RelativeGlob =
    new RelativeGlobImpl(matchers.map(Matcher.apply).toList)
  ...
  implicit def stringToRelativeGlob(glob: String): RelativeGlob = Glob(glob) match {
    case r: RelativeGlob => r
    case _ =>
      throw new IllegalArgumentException(
        s"Tried to construct a relative glob using an absolute path $glob")
  }
  ...
  private[file] class SingleFileFunctionMatcher(private val base: Path,
  ...
  private final class GlobMatcher(override val glob: String) extends SingleComponentMatcher {
    ...
    private[this] val excludeHiddenFiles = prefix == "glob" && pattern.startsWith("*")
    override def matches(path: Path): Boolean =
      matcher.matches(path) && !(excludeHiddenFiles && path.getFileName.toString.startsWith("."))
    override def equals(o: Any): Boolean = o match {
...
private[sbt] object FileGlobApi {
  private[sbt] final class FileGlobBuilder(val file: File) extends AnyVal {
    ...
    def glob(filter: FileFilter): Glob = filter match {
      case AllPassFilter => Glob(file.toPath, AnyPath)
      case f             => Glob(file.toPath, convert(f))
    }
    def *(filter: FileFilter): Glob = glob(filter)
    def globRecursive(filter: FileFilter): Glob = filter match {
      case AllPassFilter => Glob(file.toPath, RecursiveGlob)
      case f             => Glob(file.toPath, RecursiveGlob / convert(f))
    }
